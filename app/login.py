...
def check_login():
    ...
    if 'api' in request.url or endpoint in ALLOWED_ENDPOINTS:
        return
    ...
    if not user_sql.is_user_active(user_id):
        abort(401)

    try:
        roxywi_auth.check_login(user_id)
    except Exception:
        abort(401)
...
def login_page():
    ...
    if request.method == 'POST':
        ...
        next_url = request_json.get('next')
        login = checkAjaxInput(request_json.get('login'))
        ...
        try:
            ...
            return (
                roxywi_common.handle_json_exceptions(
                    e,
                    'Cannot check login password'
                ),
                401,
            )
